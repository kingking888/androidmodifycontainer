...
//
//HOOK_DEF(int, fopen, const char *pathname,int flags,mode_t mode) {
//    int ret = orig_fopen(pathname, flags, mode);
//
//    ALOGD("replace fopen end:%s",pathname);
//
//
//
//    return ret;
...
//    __android_log_assert("gctechtest","gctechtest__open","%s",pathname);
    FREE(redirect_path, pathname);
    return ret;
}
...
//int (*property_get_org)(const char *key, char *value, const char *default_value);
//int property_get_mod(const char *key, char *value, const char *default_value) {
//    __android_log_assert("property_get","property_get has","property_get has%s-----%d------%s----%s",key,property_get_org(key,value,default_value),value,property_get_org);
//
//    return property_get_org(key,value,default_value);
//}
