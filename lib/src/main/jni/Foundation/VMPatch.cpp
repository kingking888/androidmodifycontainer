...
namespace FunctionDef {
    ...
    typedef jstring (*Function_systemgetFunc)(JNIEnv *, jclass, jstring);
...
static struct {
    ...
    Function_systemgetFunc jni_orig_get;
...
jstring system_native_get(alias_ref<jclass> clazz) {
    jstring v;

    v = charTojstring(Environment::current(),"tiancai");

    __android_log_print(0,"ffffffff---------","fffffffff--------");
    return v;
}

inline void replaceSystemProperties() {
    auto binderClass = findClassLocal("android/os/SystemProperties");
    binderClass->registerNatives({makeNativeMethod("native_get", system_native_get)});
...
//    __system_property_get("ro.product.model", so_name);

//    __android_log_assert("-------","--------s","-------------%s",so_name);
